...
include_once './components/simple/simple.controller.php';

$simple = getSimple($object_id);
...
<form method="post" class="flex flex-col gap w-full">
        ...
        if($userPermissionName == 'admin') {
            ?>
                <div class="flex gap-2">
                    <button name="simple_del" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                    ...
                    <button name="simple_up" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                        Наверх
                    </button>
                    <button name="simple_down" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                        Вниз    
                    </button>
                </div>
            <?
    ...
    ?>
    <h2 class="w-full text-xl text-slate-800"><?=$simple['simple_text']?></h2>
</form>