...
include_once './components/context/context.controller.php';
include_once './components/context/context.action.php';

$context = getContext($object_id);
...
if ($lang == 'kz') {
    ...
    foreach ($sentences as $sentence) {
        ...
        if ($isPunctuation) {
            ...
            $content .= '<span class="cursor-pointer bg-slate-100 m-0.5 p-0.5 rounded duration-150 hover:bg-orange-100" onclick="speak(this, 1)">'.$sentence.'</span>';
            $isPunctuation = true;
        }
    }
}
...
<form method="POST" class="flex flex-col gap-4 w-full items-center">
    <?
        if($userPermissionName == 'admin') {
            ?>
                <div class="flex gap-2">
                    <button name="context_del" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                    ...
                    <button name="context_up" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                        Наверх
                    </button>
                    <button name="context_down" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                        Вниз    
                    </button>
                </div>
            <?
    ...
    ?>
    <h2 class="w-full text-2xl text-slate-900 font-medium text-center"><?=$context['context_title']?></h2>
    <div>
        <?
            if(!empty($context['context_image_url'])) {
                $imageUrl = $context['context_image_url'];
                ?>
                <div class="float-left pr-4 align-middle max-w-96" width="50%">
                    <img width="100%" src="<?=$imageUrl?>" />
                    <a href="<?=$imageUrl?>" target="_blank" class="underline text-gray-600 hover:text-gray-500">Источник изображения</a>
                </div>
                <?
            }
        ?>
        <p class="w-full text-xl text-slate-700 align-middle max-w-7xl">
            <?=$content?>
        </p>
    </div>
</form>