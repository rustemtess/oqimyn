...
include_once './components/read/read.controller.php';
if(!isset($userPermissionName) && $userPermissionName == null) {
    $userPermissionName = 'none';
}

?>

<div class="words font-sans subpixel-antialiased flex flex-col flex-wrap gap-2 text-xl w-full">
    <?
    if($userPermissionName == 'admin') {
        ?>
        <form method="POST" class="flex gap-2">
            <button name="read_del" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
            ...
            <button name="read_up" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                    Наверх
            </button>
            <button name="read_down" value="<?=$object_id?>" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-fit h-fit hover:bg-orange-500">
                    Вниз    
            </button>
        </form>
		<?
	...
	?>
        ...
        <?
            $readBase = getReadBase($object_id);
            foreach($readBase as $read) {
                if($read != end($readBase)) {
                    ?>
                        <p class="bg-slate-200 p-1 cursor-pointer rounded"><?=$read['read_base_word']?></p>,
                    <?
                }else {
                    ?>
                        <p class="bg-slate-200 p-1 cursor-pointer rounded"><?=$read['read_base_word']?></p>
