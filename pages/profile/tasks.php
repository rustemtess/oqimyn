        ...
        <section id="left-section" class="flex flex-col bg-orange-50 border-slate-100 divide-solid border-r px-6 py-4 gap-4 w-full max-w-md overflow-y-auto">
            ...
            <div class="flex flex-col w-full items-center mt-10 gap-2">
                ...
                <div class="flex w-full px-4 py-3 rounded-lg duration-200 justify-between gap-3 bg-white">
                    ...
                    <a class="text-left cursor-default w-full text-xl text-slate-700">Все курсы</a>
                </div>
                <div class="flex w-full px-4 py-2 rounded-lg duration-200 justify-between gap-3 hover:bg-white">
        ...
        <section class="flex flex-col gap-1 w-full overflow-auto relative mb-6">
            ...
            <div class="p-4 pt-3 border-slate-100 border-b flex justify-between items-center sticky top-0 w-full z-10" style="backdrop-filter: blur(8px); background-color: rgba(255, 255, 255, 0.8);">
                <div class="flex items-center gap-3">
                    ...
                    <h3 id="page-title" class="text-2xl">Все курсы</h3>
                </div>
            ...
            <div class="px-4">
                ...
                <div class="flex justify-center gap-4 p-2 py-4 flex-wrap">
                    
                    <?

                        $lessons = $db->query(
                            "SELECT
                            contexts.context_title,
                            contexts.context_content,
                            pages.page_id,
                            pages.tab_id,
                            tabs.tab_title,
                            ROW_NUMBER() OVER (PARTITION BY pages.tab_id ORDER BY pages.page_id) AS page_count
                        FROM
                            pages
                        JOIN
                            (
                                ...
                                GROUP BY
                                    page_id
                            ) AS unique_pages_content ON pages.page_id = unique_pages_content.page_id
                        JOIN
                            contexts ON contexts.context_id = unique_pages_content.object_id
                        JOIN
                            tabs ON pages.tab_id = tabs.tab_id;
                        "
                        )->fetch_all(MYSQLI_ASSOC);
                        if(isset($_POST['next']))
                            loadPage(0, getFirstPage());
                        if(!$lessons) {
                            if($userPermissionName == 'admin') {
                                ?>
                                    <form method="POST">
                                        <button name="next" class="cursor-pointer bg-orange-600 px-3 py-2 text-slate-50 rounded text-lg w-inherit h-fit hover:bg-orange-500">Начать создание</button>
                                    </form>
                                <?
                            }else {
                                ?>
                                    <p>В данный момент уроков нету</p>
                                <?
                            }
                        }
                        $ignore = ['contexts', 'descriptions', 'simples', 'tasks_read'];
                        $dbQueryText = '';
                        foreach($ignore as $ingoreText) {
                            $dbQueryText .= "AND pages_content.object_table != '$ingoreText'";
                        }
                        foreach($lessons as $lesson) {
                            $tabId = $lesson['tab_id'];
                            $page_id = $lesson['page_id'];
                            $tabTitle = $lesson['tab_title'];
                            $pageCount = $lesson['page_count'];
                            $progress = $db->query(
                                "SELECT (SELECT COUNT(pages_content.object_id)
                                FROM pages_content, pages
                                WHERE pages_content.page_id = $page_id 
                                AND pages.page_id = $page_id $dbQueryText) AS FULL,
                                (SELECT COUNT(tasks_completed.completed_id)
                                FROM pages_content, pages, tasks_completed
                                WHERE pages_content.page_id = $page_id 
                                AND pages.page_id = $page_id $dbQueryText
                                AND tasks_completed.completed_task_id = pages_content.object_id AND tasks_completed.user_id = $userId AND tasks_completed.completed_table = pages_content.object_table) AS DO"
                            )->fetch_assoc();

                            include './components/lesson/index.php';
                        }
